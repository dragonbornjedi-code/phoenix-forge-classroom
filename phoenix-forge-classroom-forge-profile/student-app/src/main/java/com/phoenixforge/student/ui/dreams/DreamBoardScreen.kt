...
import com.phoenixforge.student.domain.dreams.DreamBoardCatalog
import com.phoenixforge.student.domain.model.DreamEntry

@Composable
fun DreamBoardScreen(viewModel: DreamBoardViewModel = hiltViewModel()) {
    ...
    LazyColumn(
        ...
        item {
            ...
            Text(
                "Short-term and long-term dreams you choose yourself. Your board starts empty.",
...
private fun WantSuggestionDialog(
    ...
    AlertDialog(
        ...
        confirmButton = { Button(onClick = onYes) { Text("Yes") } },
        dismissButton = { OutlinedButton(onClick = onNo) { Text("No, I'll write my own") } }
    )
}
...
private fun CustomDreamDialog(
    ...
    AlertDialog(
        ...
        confirmButton = {
            Button(onClick = onSave, enabled = text.isNotBlank()) { Text("Add to board") }
        },
        dismissButton = { TextButton(onClick = onDismiss) { Text("Cancel") } }
    )
}
